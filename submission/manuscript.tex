...
specific genera increased RR for carcinoma over diversity metrics we
assessed whether the bacterial community was better at classifying
disease versus only a select group of genera based on RR and P-value
significance and tested this using two approaches. The first approach
used genus level data and tested whether there were any differences in
AUC between all genera and selected genera during training on one study
or testing on all other studies. The second approach used OTU level data
and tested whether there was a generalized decrease in the 10-fold cross
validation (CV) model AUC across studies when comparing all OTUs versus
only OTUs that taxonomically classified to selected genera.
...
In stool the most common genera in the full genera-based models belonged
to genera such as \emph{Ruminococcus}, \emph{Bacteroides}, and
...
be key members of the community that might be studied further to
...
tissue samples coupled with potential contamination could ultimately
...
to be important model component across studies. Suggesting that even
