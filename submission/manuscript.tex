...
The focus on biomarkers within stool makes sense since the current gold
standard for diagnosis is a colonoscopy which can be time-consuming and
is not without risks. Stool offers an easy and cost-effective way in
which to stratify risk within the population. Although stool represents
an easy and less invasive way to assess risk, it is not clear how
reflective this sample is to what is actually happening on the adenoma
or carcinoma. Some studies have begun to try and assess this in health
and disease but are limited by their sample size {[}12,13{]}. Since the
stool possibly represents an agregate community of the overall
gastrointestinal (GI) tract, sampling the adenoma or carinoma directly
might do a better job at identifying important microbes and could be
drastically different then what is identified in stool. Sampling the
tissue directly may provide clearer answers but it is not without
problems. Due to bowel prep the communities left for sampling may not be
the truly resident microbes but rather a collection of what is able to
keep adhered to the mucosa. Further, these samples would contain much
more host DNA then microbial DNA. Low biomass samples are very difficult
to work with and results can end up being study dependent due to the
randomness of contamination {[}14{]}. Due to these variables, one
...
important genera to be focusing on since many studies microbiota based
studies identify a collection of oral microbes rather than single genera
{[}16,17{]}. Based on this discrepency, the second question we want to
answer with this meta-analysis is if there really is one dominant
CRC-associated genera that can be identified across studies.

The heavy focus on the identification of biomarkers has mostly dominated
the study of CRC and has had an unattended consequence of reducing the
focus on changes that occur within the underlying resident community.
This has been borne out by the majority of previous studies, both within
stool and tissue, that have focused predominately on biomarkers as well
as the only meta-analysis investigating this area {[}11{]}. Since it has
not been investigated in detail the final question that this
...
area under the curve (AUC). We observed that the community changes as
...
genera are used versus full community of OTUs {[}Figure 4 \& 5{]}. This
decrease is observed in both adenoma and carcinoma groups {[}Figure 4 \&
5{]} with the largest difference in median AUC for stool being in
carcinoma classification {[}Figure 4B{]} and for tissue being in adenoma
{[}Figure 5A{]}.
...
observations, the early changes in the community may give rise to
initial polyp formation via interactions with the host. These changes
then create new niches in which any one of the CRC-associated genera
could gain a foothold, exacerbating the initial changes in community and
facilitating the transition from adenoma to carcinoma via a
driver-passenger type mechanism.
...
being the most prevalent in the top 10 across studies. Unlike in stool,
...
majority of the results seem to be study specific with many top 10 taxa
being present only in a single study. One other potentially worrying
sign is the presence of \emph{Propionibacterium} within the top 10 for
the genera and OTU models and could be a marker of contamination. The
low biomass of these samples coupled with potential contamination might
be a possible reason why the tissue results seem to be more sporadic
then the stool results.
