...
with adenoma and there were 6 taxa with significant ORs associated with
...
data from both feces (n=1737) and colon tissue (435 samples from NA
...
included \emph{Dorea} (OR=0.34 (0.21 - 0.57)), \emph{Blautia} (OR=0.41
(0.25 - 0.67)), and \emph{Weissella} (OR= ( - )). Mouth-associated
genera were not significantly associated with a higher OR for carcinoma
in tissue samples {[}Table S4{]}. For example, \emph{Fusobacterium} had
an OR of 3.98 (1.19 - 13.24); however, due to the small number of
studies and considerable variation in the data, the Benjimani-Hochberg
corrected P-value was 0.93 {[}Table S4{]}. It is interesting to note
that \emph{Ruminococcus} and members of \emph{Clostridium XI} in fecal
samples and \emph{Dorea} and \emph{Blautia} in tissue had ORs that were
...
overlap in the taxa with significant OR between fecal and tissue
samples.
...
Likewise, in unmatched tissue samples the 6 taxa with significant OR
...
best predictor of carcinomas and its median AUC was only 0.635. These
...
samples, the combined model had an AUC of 0.88, which was significantly
...
\textless{} 0.13). Both \emph{Dorea} and \emph{Blautia} were the most
...
AUC=0.555 (range: 0.367 - 0.971)) {[}Figure 5A \& S2A{]}. In contrast,
...
the AUC of studies was an average 22.86\% higher when we used all of the
taxa rather than the 3 taxa with significant ORs (one-tailed paired
T-test; P-value=0.053). For the models trained using data from unmatched
...
with significant ORs (one-tailed paired T-test; P-value=0.945) and those
using all taxa (one-tailed paired T-test; P-value=0.173) {[}Figure 4{]}.
...
fecal samples and 435 tissue samples remained in the analysis {[}Tables
1 and 2{]}.
...
\midrule
\endhead
Burns & SRA & V5-6 & 15 & 0 & 13\tabularnewline
Chen & SRA & V1-3 & 9 & 0 & 9\tabularnewline
Dejea & SRA & V3-5 & 15 & 0 & 13\tabularnewline
Flemer & Author & V3-4 & 103 & 37 & 94\tabularnewline
Geng & SRA & V1-2 & 8 & 0 & 8\tabularnewline
Lu & SRA & V3-4 & 20 & 20 & 0\tabularnewline
Sanapareddy & Author & V1-2 & 38 & 33 & 0\tabularnewline
\bottomrule
\end{longtable}
