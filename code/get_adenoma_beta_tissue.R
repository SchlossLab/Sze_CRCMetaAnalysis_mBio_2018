### Get Tissue Beta Diversity (Bray-Curtis) - Polyp only
...
get_metadata_data <- function(i,dataName){
  ...
  filtered_temp_data <- tempdata %>% filter(study == i) %>% 
    ...
    mutate(id = as.character(id), 
           is_adn = factor(ifelse(
             disease == "polyp", invisible("Y"), invisible("N")), levels = c("Y", "N")))
...
unmatched_meta <- mapply(get_metadata_data, 
                         c(tissue_sets, both_sets), "tissue_unmatched", SIMPLIFY = F)


matched_meta <- list(lu = get_metadata_data("lu", "tissue_matched"))


# Reorder the distance matrix to match the the metadata length
reordered_unmatched_dist <- mapply(reorder_dist, c(tissue_sets, both_sets), 
                                   "unmatched_meta", SIMPLIFY = F)

reordered_matched_dist <- list(lu = reorder_dist("lu", "matched_meta"))

# Generate the total n in each group
unmatched_total_compared <- lapply(unmatched_meta, function(x) table(x$is_adn)) %>% 
  bind_cols() %>% t() %>% as.data.frame() %>% mutate(study = rownames(.)) %>% 
  rename(adn_n = V1, not_adn_n = V2)

matched_total_compared <- lapply(matched_meta, function(x) table(x$is_adn)) %>% 
  bind_cols() %>% t() %>% as.data.frame() %>% mutate(study = rownames(.)) %>% 
  rename(adn_n = V1, not_adn_n = V2)

# Get comparisons
beta_perm_unmatched_results <- t(mapply(make_adonis_test, c(tissue_sets, both_sets), 
                                        "reordered_unmatched_dist", "unmatched_meta")) %>% 
  as.data.frame() %>% mutate(study = rownames(.)) %>% 
  inner_join(unmatched_total_compared, by = "study")


beta_perm_matched_results <- t(mapply(make_adonis_test, "lu", 
...
matched_bray_casetocontrol <- list(lu = mapply(get_bray_dist, "lu", 
                                     "reordered_matched_dist", "matched_meta", "control", "polyp"))

matched_bray_controls <- list(lu = mapply(get_bray_dist, "lu", 
                                "reordered_matched_dist", "matched_meta", "control", "polyp", 
                                getCont = TRUE, getCase = FALSE))

matched_bray_cases <- list(lu = mapply(get_bray_dist, "lu", 
                             "reordered_matched_dist", "matched_meta", "control", "polyp", 
...
bray_distance_matched_test_cont <- t(as.data.frame(mapply(run_wilcox, "lu",  
...
bray_distance_matched_test_cases <- t(as.data.frame(mapply(run_wilcox, "lu", 
...
write.csv(beta_perm_unmatched_results, "data/process/tables/beta_perm_adn_unmatched_tissue_summary.csv", 
          row.names = F)

write.csv(beta_perm_matched_results, "data/process/tables/beta_perm_adn_matched_tissue_summary.csv", 
          row.names = F)

write.csv(bray_distance_matched_test_cont, 
          "data/process/tables/bray_adn_matched_v_cont_tissue_summary.csv", 
          row.names = F)

write.csv(bray_distance_matched_test_cases, 
          "data/process/tables/bray_adn_matched_v_cases_tissue_summary.csv", 
          row.names = F)
