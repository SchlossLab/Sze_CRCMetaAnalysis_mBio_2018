### Test each increasing positive amount against 0 for RR of Adenoma
### Tissue
### Marc Sze
...
##############################################################################################
############### List of functions used to analyze the data ###################################
##############################################################################################
...
mod_specific_genera_list <- sapply(c(both_sets, tissue_sets), 
                                   function(x) get_select_group_totals(x, crc_genera, 
                                                                       specific_genera_list), simplify = F)
# Remve all cancer samples
final_specific_genera_list <- sapply(c(both_sets, tissue_sets), 
                                     function(x) 
                                       filter(mod_specific_genera_list[[x]], disease != "cancer"), simplify = F)
...
test <- sapply(count_names, 
               function(x) overall_counts[[x]] = as.data.frame(t(sapply(c(both_sets, tissue_sets), 
                      function(y) generate_counts(y, select_counts[x], 
                                                  final_specific_genera_list)))), simplify = F)
...
write_csv(pooled_results, "data/process/tables/adn_select_genus_inc_4_tissue.csv")
