...
shared <- read.table("data/process/tissue/geng/combined.unique.good.filter.unique.precluster.pick.pick.an.unique_list.shared", header=T)

metadata <- read.csv("data/process/tissue/geng/GengData.csv", stringsAsFactors=F)
...
write.table(shared, file="data/process/tissue/geng/geng.shared", quote=F, sep='\t', row.names=F)

write.table(metadata, file="data/process/tissue/geng/geng.metadata", quote=F, sep='\t', row.names=F)
