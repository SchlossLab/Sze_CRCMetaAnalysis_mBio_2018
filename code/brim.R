...
shared <- read.table("data/process/brim/combined.trim.unique.good.filter.unique.precluster.pick.pick.an.unique_list.shared", header=T)

metadata <- read.csv("data/process/brim/brim.metadata.csv", stringsAsFactors=F)
