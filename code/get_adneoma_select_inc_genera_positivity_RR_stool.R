...
final_specific_genera_list <- sapply(stool_sets, 
                                     function(x) 
                                       filter(mod_specific_genera_list[[x]], disease != "cancer"))
...
test <- sapply(count_names, 
               function(x) overall_counts[[x]] = as.data.frame(t(sapply(stool_sets, 
...
write_csv(pooled_results, "data/process/tables/adn_select_genus_inc_4_stool.csv")
