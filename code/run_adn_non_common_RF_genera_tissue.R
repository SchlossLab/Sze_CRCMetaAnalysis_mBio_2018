...
selected_rf_training_models <- sapply(
  c(both_sets, tissue_sets), 
  function(x) make_rf_model(select_unmatched_matched_genera_list[[x]]), simplify = F)

# Generate the data from testing on other studies
selected_rf_study_test <- sapply(
  c(both_sets, tissue_sets), 
  function(x) get_select_test_data(x, c(both_sets, tissue_sets), 
                                   selected_rf_training_models, 
                                   select_unmatched_matched_genera_list), simplify = F)


# Generate pvalue comparisons between train and test sets
selected_train_test_pvalues <- sapply(
  c(both_sets, tissue_sets), 
  function(x) make_model_comparisons(x, c(both_sets, tissue_sets), selected_rf_study_test), simplify = F)
