...
adn_tissue_graph <- adn_tissue %>% 
  ...
  mutate(type = factor(type, 
         ...
         study = factor(study, 
                        levels = c("flemer", "lu", "sana"), 
                        labels = c("Flemer", "Lu", "Sanapareddy"))) %>% 
  ...
  scale_color_manual(values = c('#ED9121', '#8B7500', '#8EE5EE')) + 
...
crc_tissue_graph <- crc_tissue %>% 
  ...
  mutate(type = factor(type, 
         ...
         model_type = factor(model_type, 
                             ...
                             labels = c("Significant\nOR Taxa", "All Taxa", "All OTUs")), 
         study = factor(study, 
                        levels = c("burns", "chen", "dejea", "flemer", "geng"), 
                        labels = c("Burns", "Chen", "Dejea", "Flemer", "Geng"))) %>% 
  ...
  scale_color_manual(values = c('#453581FF', '#CD6889', '#1874CD', '#ED9121', '#EEDC82')) + 
